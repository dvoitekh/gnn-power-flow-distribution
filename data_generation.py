...
def add_virtual_slack_edges(data: Data, topo: GraphTopology,
                              hub_mode: str = "slack") -> Data:
    """Add shortcut edges from a hub bus to all other buses.

    This reduces the effective graph diameter to at most 2 hops from any
    node to the hub. Three hub-selection modes are supported for ablation:

    - ``"slack"`` (default): hub is the slack bus, the physically meaningful
      voltage reference of the radial feeder. This is the method proposed in
      the paper.
    - ``"random"``: hub is a uniformly random non-slack bus, fixed once per
      graph using a deterministic seed derived from the bus list. Tests
      whether physical meaning of the hub matters.
    - ``"new_node"``: hub is a brand-new node appended to the graph (with
      zeroed input features), connected to every existing node. This is the
      generic virtual-node construction of Gilmer et al. (2017).

    Args:
        data: PyG Data object
        topo: GraphTopology with ext_grid_buses info
        hub_mode: one of "slack", "random", "new_node"

    Returns:
        New Data object with additional edges (and possibly one extra node).
    """
    bus_to_idx = {b: i for i, b in enumerate(topo.bus_indices)}
    slack_indices = [bus_to_idx[b] for b in topo.ext_grid_buses if b in bus_to_idx]

    if hub_mode == "new_node":
        num_nodes = data.x.shape[0]
        # Append a virtual node with zero features
        zero_feat = torch.zeros(1, data.x.shape[1], dtype=data.x.dtype)
        new_x = torch.cat([data.x, zero_feat], dim=0)
        # Append a zero target row as well (will be masked out in evaluation)
        zero_y = torch.zeros(1, data.y.shape[1], dtype=data.y.dtype)
        new_y = torch.cat([data.y, zero_y], dim=0)
        hub_idx = num_nodes  # the new node's index
        new_src, new_dst = [], []
        for i in range(num_nodes):
            new_src.extend([hub_idx, i])
            new_dst.extend([i, hub_idx])
        ...
        return Data(x=new_x, edge_index=combined_edge_index,
                    edge_attr=combined_edge_attr, y=new_y)

    if hub_mode == "random":
        num_nodes = data.x.shape[0]
        # Deterministic random hub per graph, reproducible across runs
        rng = torch.Generator()
        rng.manual_seed(hash(tuple(topo.bus_indices)) % (2**31))
        non_slack = [i for i in range(num_nodes) if i not in slack_indices]
        if not non_slack:
            return data
        idx_in_list = torch.randint(0, len(non_slack), (1,), generator=rng).item()
        hub_indices = [non_slack[idx_in_list]]
    elif hub_mode == "slack":
        if not slack_indices:
            return data
        hub_indices = slack_indices
    else:
        raise ValueError(f"Unknown hub_mode: {hub_mode!r}. "
                         f"Choose from: slack, random, new_node")
    ...
    for hub_idx in hub_indices:
        for i in range(num_nodes):
            ...
            edge_mask = (data.edge_index[0] == hub_idx) & (data.edge_index[1] == i)
