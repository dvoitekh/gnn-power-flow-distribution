...
def draw_feeder(ax, title, virtual=False):
    ...
    if virtual:
        ...
        # Choose rad so that arc peak heights are staircased and bounded
        # below the title — peak height ≈ rad * dx / 2.
        peak_targets = {1: 0.30, 2: 0.45, 3: 0.60, 4: 0.75, 5: 0.85, 6: 0.95}
        for i in range(1, 7):
            dx = xs[i] - xs[0]
            rad = -2.0 * peak_targets[i] / dx
